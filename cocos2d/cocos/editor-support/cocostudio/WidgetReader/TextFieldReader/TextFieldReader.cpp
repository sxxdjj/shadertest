...
namespace cocostudio
{
    ...
    void TextFieldReader::setPropsFromBinary(cocos2d::ui::Widget *widget, CocoLoader *cocoLoader, stExpCocoNode* cocoNode)
    {
        ...
        for (int i = 0; i < cocoNode->GetChildNum(); ++i) {
            ...
            CC_COLOR_PROPERTY_BINARY_READER
                ...
                textField->setPlaceHolder(CC_LOCALIZATION(value));
            }else if(key == P_Text){
                textField->setString(CC_LOCALIZATION(value));
    ...
    void TextFieldReader::setPropsFromJsonDictionary(Widget *widget, const rapidjson::Value &options)
    {
        ...
        if (ph)
        {
            textField->setPlaceHolder(CC_LOCALIZATION(DICTOOL->getStringValue_json(options, P_PlaceHolder,"input words here")));
        }
        textField->setString(CC_LOCALIZATION(DICTOOL->getStringValue_json(options, P_Text,"Text Tield")));
...
//        }
    ...
    void TextFieldReader::setPropsWithFlatBuffers(cocos2d::Node *node, const flatbuffers::Table *textFieldOptions)
    {
        ...
        textField->setPlaceHolder(CC_LOCALIZATION(placeholder));
        
        std::string text = options->text()->c_str();
        textField->setString(CC_LOCALIZATION(text));
